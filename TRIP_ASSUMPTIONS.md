...
- The main page shows only the current detailed day-by-day itinerary, with a full-trip overview map and a compact route map inside each applicable day accordion. Dates belong in the accordion heading rather than a separate left-hand weekday column. The separate `excursions.html` page is the option-selection surface; each option states approximate round-trip distance from Tolo, driving time, total duration, pace, and its main trade-off.
